...
import static agro.Preñes.txtfecha;
import static agro.Preñes.txthembra;
import static agro.Preñes.txtidHembra;
import static agro.Preñes.txttoro;
...
public class BuscarServicio extends javax.swing.JFrame {
    ...
    private void cargaTabla() {
        m.setRowCount(0);
        String sql = "SELECT s.id, s.fechaia, s.animalid, a.Nombre nom_hembra, s.sementalid, t.Nombre nom_semental\n"
                + "FROM  servicioia AS s \n"
                + "INNER JOIN animal AS a\n"
                + "ON s.animalid = a.id\n"
                + "INNER JOIN semental AS t\n"
                + "ON s.sementalid = t.id";
        String columna = "id";
        ...
        try {
            ...
            while (conn.resultado.next()) {
                m.addRow(new Object[]{conn.resultado.getInt("id"),
                    ...
                    conn.resultado.getString("nom_hembra"),
                    conn.resultado.getString("sementalid"),
                    conn.resultado.getString("nom_semental"),
                    conn.resultado.getString("fechaia")
                });

            }
        } catch (SQLException ex) {
            Logger.getLogger(BuscarServicio.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void initComponents() {
        ...
        Lista.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "id", "Hembra", "Nombre_Hembra", "Semental", "Nombre_Semental", "FechaIA"
            }
        ) {
        ...
        if (Lista.getColumnModel().getColumnCount() > 0) {
            ...
            Lista.getColumnModel().getColumn(1).setPreferredWidth(5);
            Lista.getColumnModel().getColumn(3).setPreferredWidth(5);
            Lista.getColumnModel().getColumn(5).setPreferredWidth(8);
        }
    ...
    private void BuscarSementales() {
        String sql = "select * from servicioia where id = " + Lista.getValueAt(Lista.getSelectedRow(), 0).toString();
        System.out.println(sql);
        conn.traeDatos(sql);
        try {
            if (conn.resultado.next()) {
                txtidHembra.setText(conn.resultado.getString("id"));
                txtfecha.setText(conn.resultado.getString("fechaia"));
                txthembra.setText(conn.resultado.getString("animalid"));
                txttoro.setText(conn.resultado.getString("sementalid"));
